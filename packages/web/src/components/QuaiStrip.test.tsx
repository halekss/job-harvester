...
// Offre déjà classée dans une voie du Pipeline — pour simuler une carte glissée depuis le
// Pipeline vers le Quai (allOffers, jamais offers : le Quai lui-même ne contient que du "new").
function makeLaneOffer(id: string, status: string, eventId: string): OfferSummary {
  return { ...makeOffer(id), status, activeEvents: { [status]: eventId } };
}
...
describe("QuaiStrip", () => {
  it("shows the offer count and each offer's title", () => {
    ...
    renderWithClient(<QuaiStrip offers={[makeOffer("a"), makeOffer("b")]} allOffers={[makeOffer("a"), makeOffer("b")]} collapsed={false} onToggleCollapsed={vi.fn()} />);
  ...
  it("shows a singular count for one offer", () => {
    vi.stubGlobal("fetch", vi.fn());
    renderWithClient(<QuaiStrip offers={[makeOffer("a")]} allOffers={[makeOffer("a")]} collapsed={false} onToggleCollapsed={vi.fn()} />);
    expect(screen.getByText(/Quai · 1 nouvelle$/)).toBeInTheDocument();
  });

  it("shows an empty-state message when there are no offers", () => {
    vi.stubGlobal("fetch", vi.fn());
    renderWithClient(<QuaiStrip offers={[]} allOffers={[]} collapsed={false} onToggleCollapsed={vi.fn()} />);
    expect(screen.getByText("Aucune offre en attente de tri.")).toBeInTheDocument();
  });

  it("hides the offer strip when collapsed", () => {
    vi.stubGlobal("fetch", vi.fn());
    renderWithClient(<QuaiStrip offers={[makeOffer("a")]} allOffers={[makeOffer("a")]} collapsed onToggleCollapsed={vi.fn()} />);
  ...
  it("calls onToggleCollapsed when the header button is clicked", async () => {
    ...
    renderWithClient(<QuaiStrip offers={[]} allOffers={[]} collapsed={false} onToggleCollapsed={onToggleCollapsed} />);
  ...
  it("dropping a card from a Pipeline lane repositions it on the Quai by deleting its active event", async () => {
    const fetchMock = vi.fn(async () => new Response(JSON.stringify({ ok: true }), { status: 200 }));
    vi.stubGlobal("fetch", fetchMock);
    const laneOffer = makeLaneOffer("a", "interview", "evt-1");
    renderWithClient(
      <QuaiStrip offers={[]} allOffers={[laneOffer]} collapsed={false} onToggleCollapsed={vi.fn()} />,
    );

    const dataTransfer = { setData: vi.fn(), getData: vi.fn(() => "a"), dropEffect: "", effectAllowed: "" };
    const dropZone = screen.getByLabelText("Quai de réception");
    fireEvent.dragOver(dropZone, { dataTransfer });
    fireEvent.drop(dropZone, { dataTransfer });

    await waitFor(() => {
      expect(fetchMock).toHaveBeenCalledWith("/offers/a/events/evt-1", expect.objectContaining({ method: "DELETE" }));
    });
  });

  it("does not delete anything when a card already on the Quai is dropped back onto it", async () => {
    const fetchMock = vi.fn(async () => new Response(JSON.stringify({ ok: true }), { status: 200 }));
    vi.stubGlobal("fetch", fetchMock);
    const quaiOffer = makeOffer("a");
    renderWithClient(
      <QuaiStrip offers={[quaiOffer]} allOffers={[quaiOffer]} collapsed={false} onToggleCollapsed={vi.fn()} />,
    );
